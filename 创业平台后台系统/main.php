...
<div class="layui-layout layui-layout-admin">
    <div class="layui-header">
        ...
        <ul class="layui-nav layui-layout-right">
            ...
            <li class="layui-nav-item"><a href="admin/sever_LogOut.php">退了</a></li>
        </ul>
    </div>
    ...
    <div class="layui-body">
        ...
        <div style="padding: 15px;">
                ...
                <button onclick="location.href='addPerson.php'" type="button" class="layui-btn" style="margin-top:-20px">
                    <i class="layui-icon">&#xe608;</i> 添加用户
                </button>

            <form class="layui-form-item layui-inline" method="get" action="./main.php">
                <div class="demoTable">
                    <div class="layui-inline">
                        <input value="<?=empty($search_age)?'':$search_age?>"  class="layui-input" name="search_age" id="demoReload" autocomplete="off" placeholder="请输入查询年龄" >
                    </div>
                    <select name="type" lay-verify="required" style="height: 40px;width: 100px">
                        <option value="">请选择分类</option>
                    <?php
                        foreach ($user as $item){
                            if (!empty($type)&& $type==$item['type']){
                                echo "<option selected value='{$item['type']}'>{$item['type_name']}</option>";
                            }else{
                                echo "<option value='{$item['type']}'>{$item['type_name']}</option>";
                           }
                        }
                        ?>
                    </select>
        ...
        laypage.render({
            ...
            ,jump: function(obj, first){
                ...
                if(!first){
                    //do something
                    <?php
                        $url='location.href="./main.php?curr="+obj.curr';
                        if(!empty($search_age)){
                            $url.="+'&search_title=$search_age'";
                        }
                        if(!empty($type)){
                            $url.="+'&type=$type'";
                        }
